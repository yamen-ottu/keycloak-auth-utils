...
class KeycloakSync:
    def __post_init__(self):
        self.kc_client_id = self._get_obj_by_kc_key(kc_admin.get_clients, KC_UTILS_KC_CLIENT_ID, "clientId", "id")
    ...
    def _get_obj_by_kc_key(self, kc_admin_objs_getter, obj_value, fetch_key, return_key=None, use_admin=False):
        objects = kc_admin_objs_getter(self.kc_client_id) if use_admin else kc_admin_objs_getter()
        obj = next(
    ...
    def _get_kc_entity_by_name(
        ...
        key, use_admin = ("username", False) if entity_name == "user" else ("name", True)

        return self._get_obj_by_kc_key(fetcher_func, entity_name, key, use_admin=use_admin)
    ...
    def _get_or_create_kc_entity(self, json: Dict, entity_type: str, key="name") -> Optional[Dict]:

        entity_name = json[key]
        if (
            entity := self._get_kc_entity_by_name(
...
class KeycloakPermission(KeycloakSync):
    ...
    def _validate_models(self):
        ...
        if self.desired_models_perms_map:
            print('yes', self.desired_models_perms_map)
...
class KeycloakUser(KeycloakSync):
    ...
    def create_user(self, user):
        ...
        user = self._get_or_create_kc_entity(json_user, entity_type="user", key="username")
    ...
    def _add_superadmin_roles(self):
        ...
        realm_manage_client_id = self._get_obj_by_kc_key(kc_admin.get_clients(),"realm-management", "id", "id",)
...
class KeycloakBase(KeycloakSync):
    ...
    def _validate_clients(self):
        logger.info(f"created realm ['name'] successfully ")

        """Validates and processes client list to ensure no duplicates."""
        base_clients = {"core", "frontend"}
        filtered_clients = set(self.clients)
        ...
        return list(base_clients.union(filtered_clients))
    ...
    def create_client_protocol_mapper(
        ...
        kc_admin.create_client_scope_mapper(client_scope_id, protocol_mappers[mapper_type])
    ...
    def create_client_scope(self, client_name):
        ...
        logger.info(f"created client scope {client_name} of type {client_scope} successfully")
        return client_scope

    def add_client_scope_to_client(self, client_id, client_scope_name="timezone"):
        client_scope_id = self._get_obj_by_kc_key(kc_admin.get_client_scopes, client_scope_name, "name", "id")
        payload = {
    ...
    def run_routine(self):
        try:
            ...
            for client in self.clients:
                client_type = "private" if client != "frontend" else "public"
