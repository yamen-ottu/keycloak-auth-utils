...
class KeycloakAdminSingleton:
    ...
    def __new__(cls):
        if cls._instance is None:
            cls._instance = super(KeycloakAdminSingleton, cls).__new__(cls)
    ...
    def update_realm_upconfig(self, realm_name, payload):
        ...
        data_raw = self.connection.raw_put(
            ADMIN_UI_URL.format(**params_path), data=json.dumps(payload)
        )
        return raise_error_from_response(
            data_raw, KeycloakPutError, expected_codes=[200]
        )
    ...
    def get_realm_upconfig(self, realm_name):
        ...
        return raise_error_from_response(
            data_raw, KeycloakGetError, expected_codes=[200]
        )
    ...
    def update_client_resource_server(self, client_id, payload):
        ...
        params_path = {"realm-name": self.connection.realm_name, "id": client_id}
        data_raw = self.connection.raw_put(
            CLIENT_RESOURCE_SERVER_URL.format(**params_path), data=json.dumps(payload)
        )
        return raise_error_from_response(
    ...
    def create_client_scope_mapper(self, client_scope_id, payload, skip_exists=True):
        ...
        data_raw = self.connection.raw_post(
            PROTOCOL_MAPPERS_URL.format(**params_path), data=json.dumps(payload)
        )
        return raise_error_from_response(
